// i8014W_MagicScan_Block_Cplusplus.cpp : Defines the entry point for the console application.
//
...
start:
	...
	T1=GetTickCount();
	totalScaned=0;
	ret= pac_i8014W_ReadFIFO_BlockMode(slotIndex,hexData, TargetCnt ,&readCnt);// <=== get data from FIFO ,long* dataCnt
	pac_i8014W_StopMagicScan(slotIndex);
	T2=GetTickCount()-T1; // total time for magic scan to polling data
	printf("Magic scan sample %ld data  spend time = %lu ms  \n\n\n",readCnt, T2);
	if(ret ==FIFO_LATCHED)
	{
		printf("Magic Scan FIFO Latched, Sample Rate too high , try to use Lower Sample Rate\n");
		pac_i8014W_UnLockFIFO(slotIndex);
		pac_i8014W_ClearFIFO(slotIndex);
		goto start;
	}
	if(readCnt>0)
	{
		if(TargetCnt != readCnt)
		{
			printf("Magic Scan Block Mode Error, TargetCnt %ld must be the same as readCnt %ld\n");
			printf("try again \n");
			goto start;
		}
		totalScaned = readCnt;
...
void ShowAI(void)
{
	...
	for(i=0;i<totalScaned;i++)
	{
		if((i%scanChCount)!=0)
		{
			pac_i8014W_CalibrateData(slotIndex, gainArr[i %scanChCount],hexData[i],&calibratedAI);
			printf("Arr[%d]C[%d]=F[%5.4f]  ",i,i%scanChCount,calibratedAI);
		}
		else
		{
			if(i!=0)
